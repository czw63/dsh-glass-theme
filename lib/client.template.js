...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const DEFAULT_SETTINGS = {
			...
			userBubbleAlpha: 65,   // 用户气泡不透明度 %（0-100，默认 65）
			sidebarAlpha: 50,      // 侧边栏不透明度 %
			radius: 18,            // 大圆角 px
			noiseOpacity: 5,       // 磨砂颗粒强度 %
			noiseSize: 180,        // 磨砂颗粒大小 px（噪点 tile 尺寸）
		};
		...
		function loadSettings() {
			try {
				...
				if (Number.isFinite(Number(p.userBubbleAlpha))) s.userBubbleAlpha = clamp(p.userBubbleAlpha, 0, 100);
				...
				if (Number.isFinite(Number(p.noiseSize))) s.noiseSize = clamp(p.noiseSize, 60, 400);
				return s;
...
:root {
	...
	--glass-bubble-alpha: 72%;    /* AI 气泡（玻璃透明，兼顾长文/代码可读） */
	--glass-user-bubble-alpha: 65%; /* 用户气泡（更透，玻璃感明显） */
	...
	--glass-noise-opacity: 0.05;   /* 磨砂噪点颗粒强度 */
	--glass-noise-size: 180px;     /* 磨砂颗粒大小（噪点 tile 尺寸，越大颗粒越粗） */
...
/* AI/助手消息气泡：较高不透明度（默认 72%，可调）保证正文/代码/表格可读 */
.gdEzaW_bubble {
...
/* ============ end dsh-glass-theme ============ */`;
		...
		function applySettingsToDom(settings) {
			...
			root.style.setProperty("--glass-noise-size", settings.noiseSize + "px");
			if (settings.bgImage) {
		...
		function GlassSettingsCard(props) {
			...
			return h("li", { style: styles.card, "data-open": open ? "true" : "false" },
				...
				open ? h("div", { style: styles.body },
					...
					slider("用户气泡透明度", s.userBubbleAlpha, 0, 100, 1, "%", (v) => props.update({ userBubbleAlpha: v })),
					...
					slider("磨砂颗粒强度", s.noiseOpacity, 0, 20, 1, "%", (v) => props.update({ noiseOpacity: v })),
					slider("磨砂颗粒大小", s.noiseSize, 60, 400, 10, "px", (v) => props.update({ noiseSize: v })),
