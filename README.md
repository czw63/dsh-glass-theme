...
- **全局玻璃层级**：背景插画（柔化 + 蒙层）→ 侧边栏 → 详情栏 → 主区 → 输入卡片 → 气泡（高不透明度保可读）→ 弹层（折射感最强）。
- **性能优先**：
...
- **可读性保障**：文字 token 一概不动；气泡不透明度 ≥ 80%（默认 82%）；代码块保持默认不透明；选区/焦点环清晰。
- **可视化设置卡片**：主题开关、背景图片、背景模糊、输入框磨砂、用户/AI 气泡透明度（分开控制）、侧边栏透明度、面板圆角、磨砂颗粒，实时生效、可恢复默认。
...
| **用户气泡透明度** | 50–100% | 82 | 用户气泡单独控制（基准色更深，突出用户消息） |
| AI 气泡透明度 | 50–100% | 82 | AI 气泡（可读性红线 ≥ 80%） |
...
s('--glass-bubble-alpha', '92%')     // AI 气泡（默认 82%）
s('--glass-user-bubble-alpha', '92%')// 用户气泡（默认 82%）
